...
from utils1 import make_noise, save_command_run_params
# cifar10 imgs =[3*32*32] latent [512]
# 运行此代码的命令如下
# nohup python run_train_cifar10.py --gan_type stylegan2-ada --deformator proj --out cifar10_discriminator_classifier_result2 --directions_count 128 --max_latent_dim 512 --deformator_random_init True >>train_cifar10_discriminator_classifier.log 2>&1 &
# nohup python run_train_cifar10.py --gan_type stylegan2-ada --deformator ortho --out cifar10_discriminator_classifier_result4 --max_latent_dim 512 --deformator_random_init True >>train_cifar10_discriminator_classifier_orth.log 2>&1 &
...
def main():
    ...
    G,D = load_generator(args.__dict__, weights_path)
    # stylegan2-ada  
    # G.z_dim is 512 
    print("loading Generator and Descriminater")
    print(G.dim_shift)
    # 训练一个分类器
    ...
    # elif args.shift_predictor == 'Cifar10':
    #     shift_predictor=
    classifier=None
    if(args.gan_type=='stylegan2-ada'):
        classifier = torch.hub.load(repo_or_dir="classifier/pytorch-cifar-models/",
    ...
    # max_latent_dim是浅空间最大的维度
    params.max_latent_dim = int(deformator.out_dim)

    trainer = Trainer(params, out_dir=args.out)
    trainer.train(G, deformator, shift_predictor, multi_gpu=args.multi_gpu,D=D,classifier=classifier)
